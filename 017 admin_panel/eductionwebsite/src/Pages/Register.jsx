...
function Register() {
   ...
   const handleFormSubmit = async (e) => {
      ...
      if(!userApproved) return alert('please accept terms and conditions');
   ...
   const handleOtpGen = async () => {
      ...
      try {
         const response = await axios.post('http://localhost:5200/otp/generate_otp', { email: data.email });
                  ...
                  <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                     ...
                     <form class="space-y-4 md:space-y-6" action="#" onSubmit={handleFormSubmit}>
                        <div>
                           ...
                           <input type="email" name="email" id="email" class="bg-gray-800 border border-gray-300 text-white sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required="" onChange={(e) => { setData({ ...data, email: e.target.value }) }} />
                           <span className='text-[red]'>{errors.email}</span>
                        </div>
                        <div>
                           <button type="button" disabled={otpBtn} onClick={handleOtpGen} class="w-full font-medium bg-gray-300 rounded-lg text-sm px-5 py-2.5 text-center mt-3">{btnText}</button>
                        </div>
                        <div className={`${ifOtp ? 'block' : 'hidden'}`}>
                           <div>
                              ...
                              <input type="password" name="password" id="password" placeholder="••••••••" class="bg-gray-800 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mb-3" required="" onChange={(e) => { setData({ ...data, password: e.target.value }) }} />
                           ...
                           <div>
                              ...
                              <input type="password" name="cpassword" id="cpassword" placeholder="••••••••" class="bg-gray-800 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" onChange={(e) => { setData({ ...data, cpassword: e.target.value }) }} required="" />
                              <span className='text-[red]'>{errors.cpassword}</span>
                           </div>
                           <div>
                              <label for="otp" class="block mb-2 text-sm font-medium text-white dark:text-white">OTP</label>
                              <input type="otp" name="otp" id="otp" placeholder="Enter OTP" class="bg-gray-800 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600  text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" onChange={(e) => { setData({ ...data, otp: e.target.value }) }} required="" />
                              <span className='text-[red]'>{errors.cpassword}</span>
                           </div>
                           <div class="flex items-center justify-between">
                              <div class="flex items-start mt-3">
                                 <div class="flex items-center h-5">
                                    <input id="remember" aria-describedby="remember" checked={userApproved} onChange={(e)=>{setUserApproved(e.target.checked)}} type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3 focus:ring-primary-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-primary-600 dark:ring-offset-gray-800" required="" />
                                 </div>
                                 <div class="ml-3 text-sm">
                                    <label for="remember" class="text-white dark:text-gray-300">I accept the Terms and Conditions</label>
                                 </div>
                              </div>

                           </div>
                           <button type="submit" class="w-full font-medium bg-gray-300 rounded-lg text-sm px-5 py-2.5 text-center mt-3">Create An Account</button>
                        </div>
